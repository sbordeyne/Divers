# -*- coding: utf-8 -*-
"""
Created on Sun Mar 29 13:23:42 2015
...
root.protocol("WM_DELETE_WINDOW", master.destroy)
...
btn = Button(master_foo, text="foo", command=master.destroy)
btn.pack(side=RIGHT)
nb.add(master_foo, text="foo") # add tab to Notebook

# repeat for each tab
master_bar = Frame(nb, name='master-bar')
Label(master_bar, text="this is bar").pack(side=LEFT)
btn = Button(master_bar, text="bar", command=master.destroy)
btn.pack(side=RIGHT)
nb.add(master_bar, text="shit")
